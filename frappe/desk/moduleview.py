...
def apply_permissions(data):
    ...
    for section in data:
        ...
        for item in (section.get("items") or []):
            ...
            ## restriction for chart of accounts (only see in menu for user has create permission)
            if ((item.type == "doctype" and item.name in user.can_read and item.name != "Account")
                or (item.type == "doctype" and item.name in user.can_create and item.name == "Account")
