...
class FrappeClient(object):
	...
	def insert(self, doc):
		...
		# res = self.url + "/api/resource/Trouble Ticketing System"+ frappe.as_json(doc) + self.verify,self.headers
	...
	def post_api2(self, method, params={}):
		res = self.session.post(self.url + "/api/method/" + method + "/",
			data=params, verify=self.verify, headers=self.headers)
		return self.post_process(res)
